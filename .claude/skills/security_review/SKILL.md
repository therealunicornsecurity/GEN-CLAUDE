...
description: EDI's Procedure B Step 6 AI security gate — invokes Anthropic's OFFICIAL bundled /security-review on the diff, IN ADDITION to the grep-based /security_scan. Not a reimplementation.
version: 2.0.0
tags: [security, review, ai, official, sast]
---

# /security_review — EDI wrapper around the official `/security-review`

EDI does **not** reimplement AI security analysis. This skill **invokes Anthropic's
official, bundled [`/security-review`](https://github.com/anthropics/claude-code-security-review)**
(semantic, diff-scoped vulnerability review) and folds its findings into EDI's Procedure B
Step 6 gate — **in addition to**, never instead of, the grep-based
[`/security_scan`](../security_scan/SKILL.md).

> The analysis engine is Anthropic's and versions with Claude Code. This file is only EDI's
...
1. `/security_scan` — EDI's own grep of `RULES.md §5` + `docs/security.md`. Deterministic. **Blocking.**
...
severity (HIGH / MEDIUM / LOW). **EDI policy:** a HIGH finding is blocking (same weight as a
...
`/security-review` needs `git diff`, but EDI's `.claude/hooks/PreToolCall` refuses all `git`.
Two ways to run it inside an EDI repo:
...
- uses: anthropics/claude-code-security-review@main
  with:
    ...
    # custom-security-scan-instructions: docs/security.md   # feed EDI's per-language addendum
...
- EDI does not fork or reimplement the analysis — it invokes the official bundled skill.
...
— bundled with Claude Code; source MIT. This skill is EDI's integration only: it decides
